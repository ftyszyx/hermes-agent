...
def _is_provider_active(provider: dict, config: dict) -> bool:
    ...
    if provider.get("imagegen_backend"):
        ...
        if not isinstance(image_cfg, dict):
            return backend == "fal"
        current_backend = str(image_cfg.get("backend") or "").strip()
        if not current_backend and image_cfg.get("base_url"):
            current_backend = "openai_compatible"
        current_backend = current_backend or "fal"
        if current_backend in {"openai", "openai-compat", "openai_compat"}:
            current_backend = "openai_compatible"
        configured_provider = image_cfg.get("provider")
        if backend != "fal":
